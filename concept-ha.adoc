...
자세한 내용은 을 참조하십시오 link:https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/reference-networking-aws.html#requirements-for-ha-pairs-in-multiple-azs["여러 AZs에서 Cloud Volumes ONTAP HA를 위한 AWS 네트워킹 요구사항"^].
...
AWS Local Zones 는 스토리지, 컴퓨팅, 데이터베이스 및 기타 엄선된 AWS 서비스가 대도시와 산업 영역에 가깝게 위치하는 인프라 구축입니다. AWS Local Zones를 사용하면 AWS 서비스를 더 가까운 곳으로 가져와 워크로드의 지연 시간을 개선하고 데이터베이스를 로컬에서 유지할 수 있습니다.
...
지원되는 유일한 디스크 유형은 GP2입니다.

크기가 xlarge에서 4xlarge인 다음 EC2 인스턴스 유형 제품군이 현재 지원됩니다.
...
link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/?nc=sn&loc=2["로컬 영역에서 지원되는 EC2 인스턴스 유형에 대한 최신 정보와 전체 세부 정보는 AWS를 참조하십시오"^].
