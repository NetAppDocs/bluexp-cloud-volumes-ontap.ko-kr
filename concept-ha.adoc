...
Cloud Volumes ONTAP는 AWS 로컬 영역에서 HA 구축만 지원합니다. 다음은 예제 구성입니다.
...
Cloud Volumes ONTAP는 이러한 구성만 지원합니다. Cloud Volumes ONTAP의 AWS 로컬 영역 구성에서 다른 디스크 유형이나 EC2 인스턴스를 선택하면 배포에 실패할 수 있습니다. 의 최신 정보와 전체 세부 정보는 AWS 문서를 참조하십시오link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/?nc=sn&loc=2["로컬 영역의 EC2 인스턴스 유형"^].
