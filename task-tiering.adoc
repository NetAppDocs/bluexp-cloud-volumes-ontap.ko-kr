...
+
Cloud Manager는 시스템이 생성될 때 콜드 데이터용 오브젝트 저장소를 생성합니다.  data tiering after implementing the requirements,연결 또는 사용 권한 문제가 없는 경우. 그런 다음 볼륨에 대해 데이터 계층화를 활성화해야 합니다. 경우에 따라  that tiering is enabled on aggregates,애그리게이트)를 클릭합니다.
