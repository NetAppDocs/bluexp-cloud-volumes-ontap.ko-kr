...
BlueXP는 Cloud Volumes ONTAP용으로 생성한 단일 스토리지 VM의 이름을 자동으로 지정합니다. 엄격한 명명 규칙이 있는 경우 스토리지 VM 이름을 수정할 수 있습니다. 예를 들어, 이름이 ONTAP 클러스터에 대한 스토리지 VM의 이름을 지정하는 방법과 일치할 수 있습니다.

Cloud Volumes ONTAP용 추가 스토리지 VM을 생성한 경우 BlueXP에서 스토리지 VM 이름을 바꿀 수 없습니다. System Manager 또는 CLI를 사용하여 Cloud Volumes ONTAP에서 직접 변경해야 합니다.
...
* https://["SVM 재해 복구 준비 Express 가이드"^]
* https://["SVM 재해 복구 익스프레스 가이드 를 참조하십시오"^]
