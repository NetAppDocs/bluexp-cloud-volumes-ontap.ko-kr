...
BlueXP는 다음과 같은 세 가지 방법으로 Cloud Volumes ONTAP를 업그레이드할 수 있습니다.
...
NOTE: 현재 BlueXP 사용자 인터페이스를 통해서만 최신 ONTAP 버전으로 업그레이드할 수 있습니다. BlueXP 사용자 인터페이스를 통해 특정 ONTAP 버전으로 업그레이드할 수 있는 기능은 향후 릴리스에서 제공될 예정입니다.
...
+
https://["NetApp KB: 업그레이드 이미지를 호스팅하기 위해 BlueXP를 HTTPS 서버로 구성하는 방법"^]
