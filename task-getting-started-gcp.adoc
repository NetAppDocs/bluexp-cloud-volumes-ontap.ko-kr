...
가 없는 경우 https://["커넥터"^] 그러나 계정 관리자는 계정을 만들어야 합니다. https://["Google Cloud에서 Connector를 만드는 방법을 알아보십시오"^]
...
. 데이터 계층화를 사용할 계획이라면 https://["개인 Google 액세스를 위한 Cloud Volumes ONTAP 서브넷을 구성합니다"^].
...
[role="quick-margin-para"]
Cloud Volumes ONTAP를 사용하려면 Google Cloud 서비스 계정이 두 가지 용도로 필요합니다. 첫 번째는 를 활성화하는 것입니다 link:concept-data-tiering.html["데이터 계층화"] Google Cloud에서 콜드 데이터를 저비용 오브젝트 스토리지로 계층화합니다. 두 번째는 를 활성화하는 것입니다 https://["BlueXP 백업 및 복구"^] 볼륨을 저렴한 오브젝트 스토리지에 백업
...
[role="quick-margin-para"]
https://["프로젝트에서 다음 Google Cloud API를 활성화합니다"^]. 이러한 API는 Connector 및 Cloud Volumes ONTAP를 구축하는 데 필요합니다.
...
* https://["BlueXP가 Google Cloud 사용 권한으로 수행하는 기능"^]
