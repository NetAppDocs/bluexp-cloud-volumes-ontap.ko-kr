...
[role="lead"]
HA 쌍에 대한 액세스를 지원하는 AWS 전송 게이트웨이를 설정합니다 link:reference-networking-aws.html#aws-networking-requirements-for-cloud-volumes-ontap-ha-in-multiple-azs["유동 IP 주소"] HA 쌍이 상주하는 VPC 외부에서
