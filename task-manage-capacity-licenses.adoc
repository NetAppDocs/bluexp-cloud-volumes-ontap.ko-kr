...
+
Essentials 패키지에 표시되는 용량을 더 잘 이해하려면 충전 작동 방식에 익숙해야 합니다. https://["Essentials 패키지 충전 방법에 대해 알아봅니다"].
...
* 일련 번호를 입력하려면 먼저 해야 합니다 https://["NetApp Support 사이트 계정을 BlueXP에 추가합니다"^]. 일련 번호에 액세스할 수 있는 권한이 있는 NetApp Support 사이트 계정입니다.
