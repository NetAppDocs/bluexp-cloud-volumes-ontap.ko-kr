...
Azure에서 여러 스토리지 VM이 Cloud Volumes ONTAP BYOL 방식으로 지원되며 9.9.0 릴리즈부터 애드온 라이센스가 제공됩니다. 로 이동합니다 https://docs.netapp.com/us-en/cloud-volumes-ontap/index.html["Cloud Volumes ONTAP 릴리즈 노트"^] 사용 중인 Cloud Volumes ONTAP 버전에 대해 지원되는 스토리지 VM 수를 확인하려면 다음을 수행합니다.
