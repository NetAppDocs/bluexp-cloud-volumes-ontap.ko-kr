...
Azure에서 여러 스토리지 VM이 Cloud Volumes ONTAP BYOL 방식으로 지원되며 9.9.0 릴리즈부터 애드온 라이센스가 제공됩니다. 로 이동합니다 https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/index.html["Cloud Volumes ONTAP 릴리즈 노트"^] 사용 중인 Cloud Volumes ONTAP 버전에 대해 지원되는 스토리지 VM 수를 확인하려면 다음을 수행합니다.
...
아래 구성 단계를 따르십시오. 단일 노드 시스템, iSCSI를 사용하는 HA 쌍 또는 NFS/SMB를 사용하는 HA 쌍
...
노드 1에서 데이터 LIF 액세스를 위한 IP 주소, 노드 2에서 데이터 LIF 액세스를 위한 또 다른 IP 주소, SVM(스토리지 VM) 관리 LIF를 위한 또 다른 선택적 IP 주소를 생성해야 합니다. 이 관리 LIF는 SnapCenter과 같은 관리 툴에 연결할 수 있습니다.
...
NFS 및 SMB 데이터에 사용하는 IP 주소는 로드 밸런싱 장치에 할당되어 페일오버 이벤트가 발생할 경우 IP 주소가 다른 노드로 마이그레이션될 수 있습니다.

.단계
. Azure 포털에서 * 로드 밸런서 * 서비스를 엽니다.
. HA 쌍에 대한 로드 밸런싱 장치의 이름을 클릭합니다.
. 노드 1에서 데이터 LIF 액세스를 위한 프런트엔드 IP 구성을 하나 생성하고, 노드 2에서 데이터 LIF 액세스를 위한 또 다른 프런트엔드 IP를 생성합니다(HA 쌍만 해당). 스토리지 VM(SVM) 관리 LIF를 위한 또 다른 선택적 프런트엔드 IP를 생성합니다.
+
...
다음 단계에서는 단일 노드 시스템 또는 HA 쌍에 새 스토리지 VM을 생성합니다. 노드 1의 데이터 LIF 액세스, 노드 2의 데이터 LIF 액세스를 위한 또 다른 IP 주소(HA 쌍만 해당), 스토리지 VM(SVM) 관리 LIF를 위한 또 다른 선택적 IP 주소 등에 한 IP 주소가 필요합니다. 이 관리 LIF는 SnapCenter과 같은 관리 툴에 연결할 수 있습니다.

아래 명령을 스토리지 VM의 데이터 액세스 프로토콜과 일치하는 NAS 또는 iSCSI 명령을 사용하십시오.
...
----
+
단일 노드 시스템인 경우 -failover-policy 매개 변수의 값을 _disabled_로 변경해야 합니다.

.. 다음 명령을 사용하여 노드 2에 NAS LIF를 생성합니다(HA 쌍만 해당).
+
[source, cli]
----
...
----
.. 다음 명령을 사용하여 노드 2에 iSCSI LIF를 생성합니다(HA 쌍만 해당).
+
[source, cli]
----
