...
Cloud Volumes ONTAP는 대부분의 AWS 지역에서 지원됩니다. https://["지원되는 영역의 전체 목록을 봅니다"^].

해당 지역에서 리소스를 생성하고 관리하려면 먼저 새로운 AWS 영역을 활성화해야 합니다. https://["지역 활성화 방법을 알아보십시오"^].
...
https://["AWS에서 Cloud Volumes ONTAP가 지원되는 구성입니다"^]
...
https://["AWS의 Cloud Volumes ONTAP에 대한 스토리지 제한"^]
...
--
EBS 디스크 유형입니다:: EBS 디스크 유형의 차이점은 다음과 같습니다. EBS 디스크의 사용 사례에 대한 자세한 내용은 을 참조하십시오 http://["AWS 설명서:EBS 볼륨 유형"^].
+
--
...
+
GP3 디스크를 선택할 때 BlueXP는 기본 IOPS와 처리량 값을 채워 선택한 디스크 크기를 기준으로 GP2 디스크와 동일한 성능을 제공합니다. 더 높은 비용으로 더 나은 성능을 얻기 위해 값을 늘릴 수 있지만 성능이 저하될 수 있으므로 더 낮은 값을 지원하지 않습니다. 즉, 기본값을 그대로 사용하거나 값을 늘립니다. 낮추지 마십시오. https://["GP3 디스크 및 성능에 대해 자세히 알아보십시오"^].
