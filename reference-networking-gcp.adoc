...
* VPC-0의 HA 구성 요소에 대한 하나의 규칙 세트 이러한 규칙을 통해 Cloud Volumes ONTAP에 대한 데이터 액세스가 가능합니다. <<Cloud Volumes ONTAP의 방화벽 규칙,자세한 정보>>.
* VPC-1, VPC-2 및 VPC-3의 HA 구성 요소에 대한 또 다른 규칙 세트 이러한 규칙은 HA 구성 요소 간의 인바운드 및 아웃바운드 통신에 대해 개방됩니다. <<Cloud Volumes ONTAP의 방화벽 규칙,자세한 정보>>.
...
GCP에서 HA 구성은 네 대의 VPC에 구축됩니다. VPC-0의 HA 구성에 필요한 방화벽 규칙은 입니다 <<Cloud Volumes ONTAP의 방화벽 규칙,Cloud Volumes ONTAP에 대해 위에 나열되어 있습니다>>.
