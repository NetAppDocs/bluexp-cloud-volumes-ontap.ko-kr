...
+Cloud Volumes ONTAP에 대해 Cloud Manager가 할당하는 LIF 수는 단일 노드 시스템을 구축하든 HA 쌍을 구축하느냐에 따라 다릅니다. LIF는 물리적 포트와 연결된 IP 주소입니다. SnapCenter와 같은 관리 툴을 사용하려면 SVM 관리 LIF가 필요합니다.
