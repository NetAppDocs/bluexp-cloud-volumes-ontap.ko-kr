...
Cloud Volumes ONTAP는 대부분의 Google 클라우드 지역에서 지원됩니다. https://["지원되는 영역의 전체 목록을 봅니다"^].
...
https://["GCP에서 Cloud Volumes ONTAP에 지원되는 구성입니다"^]
...
https://["GCP의 Cloud Volumes ONTAP에 대한 스토리지 제한입니다"^]
...
기계 유형:: 에서 지원되는 기계 유형을 확인합니다 http://["Cloud Volumes ONTAP 릴리즈 노트"^] 지원되는 각 시스템 유형에 대한 자세한 내용은 Google에서 확인하십시오. 워크로드 요구 사항을 시스템 유형에 대한 vCPU 및 메모리 수와 일치시킵니다. 각 CPU 코어는 네트워킹 성능을 향상시킵니다.
+
--
자세한 내용은 다음을 참조하십시오.

* https://["Google Cloud 설명서: N1 표준 컴퓨터 유형"^]
* https://["Google Cloud 설명서: 성능"^]
...
+
자세한 내용은 을 참조하십시오 https://["Google Cloud 설명서: Zonal Persistent 디스크(Standard 및 SSD)"^].
...
+
** https://["Google Cloud 설명서: Zonal Persistent 디스크(Standard 및 SSD)"^]
** https://["Google Cloud 설명서: 영구 디스크 및 로컬 SSD 성능 최적화"^]
...
TIP: 커넥터에는 시스템 디스크도 필요합니다. https://["커넥터의 기본 설정에 대한 세부 정보를 봅니다"^].
...
BlueXP에서는 Google Cloud의 고가용성(HA) 쌍을 제외하고 Cloud Volumes ONTAP에 대한 쓰기 속도 설정을 선택할 수 있습니다. 쓰기 속도를 선택하기 전에 고속 쓰기 속도를 사용할 때 정상 및 높음 설정의 차이점과 위험 및 권장 사항을 이해해야 합니다. link:concept-write-speed.html["쓰기 속도에 대해 자세히 알아보십시오"].
