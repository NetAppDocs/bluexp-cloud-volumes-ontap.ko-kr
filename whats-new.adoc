...
WORM(Write Once Read Many) 및 ARP(자율적 랜섬웨어 방어)의 내장 데이터 보호 및 보안 기능이 추가 비용 없이 모든 Cloud Volumes ONTAP 라이센스에 포함됩니다. 이 옵션은 노드 기반 및 용량 기반 라이센스 모두에 대한 신규 및 기존 BYOL 및 PAYGO/마켓플레이스 구축에 적용됩니다. 새로운 가격 모델은 단일 노드 및 고가용성(HA) 구성을 포함한 모든 클라우드 공급자 및 구축 유형에 적용됩니다.

다음과 같은 혜택을 이용할 수 있습니다.

* WORM 및 ARP가 있는 기존 계정에는 이러한 기능에 대한 비용이 더 이상 청구되지 않습니다. 이 변경 전에 해당되는 용량 사용량에 대한 비용이 청구됩니다. 웜 및 ARP는 향후 청구에서 제외됩니다.
* 기존 계정에 이러한 기능이 활성화되어 있지 않으면 추가 비용 없이 WORM 및 ARP를 선택할 수 있습니다.
* 새 계정의 경우 모든 Cloud Volumes ONTAP 제품에서 WORM 및 ARP 비용이 제거됩니다.
