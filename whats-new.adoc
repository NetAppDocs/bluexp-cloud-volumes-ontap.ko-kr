...
이 페이지에 설명된 향상된 기능은 Cloud Volumes ONTAP 관리를 지원하는 BlueXP 기능에만 적용됩니다. Cloud Volumes ONTAP 소프트웨어 자체의 새로운 기능에 대해 알아보려면 https://["Cloud Volumes ONTAP 릴리즈 노트 로 이동합니다"^]
...
https://["계정에서 사용된 용량을 확인하는 방법에 대해 알아봅니다"^].
...
https://["이 Cloud Volumes ONTAP 릴리스에 포함된 새로운 기능에 대해 자세히 알아보십시오"^].
...
에 대해 자세히 알아보십시오 https://["Azure에서 지원되는 디스크 크기입니다"^].
...
https://["NetApp 암호화 솔루션으로 볼륨을 암호화하는 방법을 알아보십시오"^].
...
https://["ONTAP에서 S3 오브젝트 스토리지 서비스를 구성 및 관리하는 방법에 대해 알아보십시오"^]
...
NDMP를 구성하고 사용하는 방법에 대한 자세한 내용은 을 참조하십시오 https://["NDMP 구성 개요"].
...
이 새로운 기능에 대한 자세한 내용은 을 참조하십시오 https://["Azure에서 고객이 관리하는 키를 사용하도록 Cloud Volumes ONTAP를 설정합니다"].
...
+
https://["충전 방법을 변경하는 방법에 대해 알아보십시오"].
...
* * Edge Cache *: 라이센스 https://["Cloud Volumes Edge 캐시"^]


https://["이러한 라이센스 패키지에 대해 자세히 알아보십시오"].
...
https://["AWS의 Elastic Volumes 지원에 대해 자세히 알아보십시오"].
...
https://["Cloud Volumes ONTAP에 대해 지원되는 전체 영역 목록을 봅니다"^]
...
https://["Elastic Volumes 지원에 대해 자세히 알아보십시오"].
...
"ec2:ModifyVolume",
----
BlueXP에 추가한 각 AWS 자격 증명 세트에 이러한 권한을 제공해야 합니다. https://["AWS에 대한 최신 커넥터 정책을 봅니다"^].
...
https://["Cloud Volumes ONTAP를 사용한 Azure 전용 링크 연결에 대해 자세히 알아보십시오"].
...
* https://["Google Cloud의 스토리지 VM 제한에 대해 자세히 알아보십시오"^]
* https://["Google Cloud에서 Cloud Volumes ONTAP용 데이터 서비스 스토리지 VM을 생성하는 방법을 알아보십시오"]
...
https://["BlueXP의 Kubernetes 데이터 관리에 대해 알아보십시오"^]
...
https://["계층화된 데이터에 대한 스토리지 클래스를 변경하는 방법에 대해 알아보십시오"].
...
https://["Google Cloud에서 Cloud Volumes ONTAP에 지원되는 구성을 봅니다"^]
...
Cloud Volumes ONTAP 시스템에 연결된 웹 브라우저에 클러스터 관리 IP 주소를 입력하여 System Manager에 연결할 수도 있습니다. https://["System Manager에 연결하는 방법에 대해 자세히 알아보십시오"].
...
https://["특정 애그리게이트에서 볼륨을 생성하는 방법에 대해 알아보십시오"].
...
https://["Cloud Volumes ONTAP의 라이센스 옵션에 대해 자세히 알아보십시오"].
...
https://["할당되지 않은 노드 기반 라이센스를 교환하는 방법에 대해 알아보십시오"].
...
로 이동합니다 https://["Cloud Volumes ONTAP 릴리즈 노트"^] 지원되는 구성에 대한 자세한 내용은 를 참조하십시오.
...
* https://["NetApp Keystone 구독에 대해 자세히 알아보십시오"^].
...
Azure의 Cloud Volumes ONTAP에서 를 사용하여 데이터가 자동으로 암호화됩니다 https://["Azure 스토리지 서비스 암호화"^] Microsoft 관리 키를 사용합니다. 그러나 이제 다음 단계를 완료하여 고객이 관리하는 암호화 키를 사용할 수 있습니다.
...
https://["AWS에서 지원되는 Cloud Volumes ONTAP 구성 보기"^].
...
----
BlueXP에 추가한 각 Azure 자격 증명 세트에 이러한 권한을 제공해야 합니다. https://["Azure의 최신 커넥터 정책을 봅니다"^].
...
https://["GCP에서 Cloud Volumes ONTAP에 대해 지원되는 구성을 봅니다"^].
...
로 이동합니다 https://["AWS 마켓플레이스 페이지를 참조하십시오"^] 가격 세부 정보를 보려면 로 이동하십시오 https://["Cloud Volumes ONTAP 릴리즈 노트"^] 이 라이센스 옵션에 대해 자세히 알아보십시오.
...
Connector에는 Azure 스냅샷을 삭제할 수 있는 새로운 권한이 필요합니다. https://["Azure의 최신 커넥터 정책을 봅니다"^].
...
이제 Cloud Volumes ONTAP는 단일 노드 시스템에서 E32s_v3 VM 유형을 사용할 때 Ultra SSD를 VNVRAM으로 사용할 수 있습니다 https://["지원되는 모든 Azure 지역에서 제공됩니다"^].
...
* https://["Azure 프라이빗 링크에 대해 자세히 알아보십시오"^]
* link:task-enabling-private-link.html["Cloud Volumes ONTAP에서 Azure 프라이빗 링크를 사용하는 방법에 대해 자세히 알아보십시오"]
