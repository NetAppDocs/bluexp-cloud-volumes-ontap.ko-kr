...
이 페이지에 설명된 향상된 기능은 Cloud Volumes ONTAP 관리를 지원하는 Cloud Manager 기능에만 적용됩니다. 별도의 릴리즈 노트에서는 Cloud Volumes ONTAP 소프트웨어 자체의 새로운 기능에 대해 설명합니다.
...
link:task-create-volumes.html["특정 애그리게이트에서 볼륨을 생성하는 방법에 대해 알아보십시오"].
...
link:task-manage-node-licenses.html#exchange-unassigned-node-based-licenses["할당되지 않은 노드 기반 라이센스를 교환하는 방법에 대해 알아보십시오"].
